...
export class SuiviDuJourPresenterImpl implements SuiviDuJourPresenter {
  ...
  private getMoyenneDesSuivis(resultat: Resultat): number[] {
    return new Array(resultat?.suivisPrecedent.datesDesSuivis.length).fill(
      resultat.suivisPrecedent.moyenneDesSuivis / 1000
    );
  }
  ...
  presente(resultat: Resultat) {
    this._viewModel({
      impactCarbonDuJour: {
        ...
        pictoSens:
          resultat?.impactCarbonDuJour.variation > 0
