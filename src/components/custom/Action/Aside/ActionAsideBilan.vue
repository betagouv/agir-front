<template>
  ...
  />
  <ActionAsideRealisee v-else :points="actionBaseViewModel.points" sous-titre="Vous avez réalisé votre bilan" />

  <template v-if="nombreActionRealise > 1">
    <hr />

    <p class="fr-mb-1w">
      <span class="text--bold"> {{ nombreActionRealise }} bilans terminés </span>
      par la communauté
    </p>
  </template>
</template>
