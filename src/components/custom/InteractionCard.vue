<template>
  <a
    v-bind="attributsDuLien"
    role="link"
    @click="props.onInteractionClick ? props.onInteractionClick(interactionViewModel) : null"
    class="interaction-card fr-grid-row fr-grid-row--middle background--white border-radius--md fr-p-2w"
  >
    <img
      :src="interactionViewModel.illustrationURL"
      class="fr-col-3 interaction-card__img border-radius--md fr-hidden fr-unhidden-md"
      alt=""
    />
    <div class="fr-col-12 fr-col-md-7 interaction-card__contenu">
      ...
      <span class="fr-icon-leaf-line fr-icon--sm fr-text--xs text--black-light"
        >+{{ interactionViewModel.nombreDePointsAGagner }}</span
      >
    </div>
    <span class="fr-col-2 text--right fr-hidden fr-unhidden-md">
      <span v-if="!interactionViewModel.estBloquee" class="text--bleu"
        >Commencer<span class="fr-icon-arrow-right-s-line" aria-hidden="true">&nbsp;</span></span
      >
